...
def create_placement(vars):
    ...
    pd.DataFrame(data=output).to_csv('results/legacy_format/gateways_Placement_Range' + str(name) + '.csv')
    ...
    pd.DataFrame(data=output).to_csv('results/legacy_format/reachable_sensors_Range' + str(name) + '.csv')
    pd.DataFrame(data=output).to_json('results/legacy_format/reachable_sensors_Range' + str(name) + '.json')
    ...
    pd.DataFrame(data=output).to_csv('results/placement_results' + str(name) + '.csv')
    pd.DataFrame(data=output).to_json('results/placement_results' + str(name) + '.json')
...
if __name__ == "__main__":
    ...
    for currfile in sensor_data_paths:
        ...
        print(zonenumber)
        configs = [i for i in range(1172, 1173, 50)]
        # create_placement([Sensors, hata(12, 15, 1), 0, 10000000, zonenumber,name])
        with Pool(10) as p:
            devices_against_collision = p.map(create_placement, [[Sensors, i, 0, 1000, zonenumber,"_"+name] for i in configs])
