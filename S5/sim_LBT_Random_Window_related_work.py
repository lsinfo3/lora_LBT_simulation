...
def check_LBT_violation_and_process(band, transmission, invalid, transmission_index, LBT_sens_period, LBT_max_retries,
    ...
    if band_active and (transmission[2]["try"] < LBT_max_retries):
        ...
        backoff = random.randrange(400,1751)
...
def simulate_collision(args):
    ...
    with open(str(args[15]) + "/collision_probability_" + str(args[13]) + "_" + str(args[14]) + ".json",
              "w") as outfile:
...
if __name__ == '__main__':
    ...
    scheduled_buffer = 11.1 * 360 + 36 + payload_size_to_time(scheduled_sync_packet_length,
                                                              12)  # ms # mit k aus figure_6_data02_slotted_RA_lesstraffic
    ...
    split_pattern = "_RES"
    output_folder = "results/collision_probability_RW"
    ...
    sensor_values = [str(curr).split(".json")[0] for curr in sensor_values]
    print(sensor_values)
    ...
    variable_key = []
    for variable_parameter in range(25, 26,10):
        for data in range(len(sensor_data_paths)):
            scenarios.append(
                [data, payload_bytes, sim_hours, scheduled_buffer, scheduled_max_time_drift,
                 scheduled_sync_packet_length, scheduled_slot_length, variable_parameter, LBT_sens_period, runs,
