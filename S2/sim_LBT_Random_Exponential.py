...
def check_LBT_violation_and_process(band, transmission, invalid, transmission_index, LBT_sens_period, LBT_max_retries,
    ...
    if band_active and (transmission[2]["try"] < LBT_max_retries):
        ...
        backoff = int(np.random.exponential(LBT_backoff_max_time) * 1000)
...
if __name__ == '__main__':
    ...
    variable_key = []
    for variable_parameter in range(400, 1820,10):
        for data in range(len(sensor_data_paths)):
            scenarios.append(
                [data, payload_bytes, sim_hours, scheduled_buffer, scheduled_max_time_drift,
                 scheduled_sync_packet_length, scheduled_slot_length, (variable_parameter/1000), LBT_sens_period, runs,
                 sensor_data_paths, type_distribution, types, sensor_values[data], (variable_parameter/1000), output_folder])
            scenario_sensor_values.append(sensor_values[data])
            variable_key.append((1/(variable_parameter/1000)))
